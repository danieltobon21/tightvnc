...
// TobonVNC fork.
//
// Indices of the two padlock images that live at the end of the toolbar strip
// (res/toolbar.bmp, 18 images of 24x24 generated by tools/make-toolbar-icons.py):
// image 16 is the closed padlock (remote input blocked) and 17 the open one.
//
static const int IMG_REMOTEINPUT_LOCKED = 16;
static const int IMG_REMOTEINPUT_ENABLED = 17;
...
bool ViewerWindow::onCreate(LPCREATESTRUCT lps)
{
  ...
  //
  // TobonVNC fork: the strip carries the 16 command images (0..15) plus the two
  // padlock states (16 locked, 17 unlocked), so only the first 16 become
  // automatic buttons.
  //
  m_toolbar.setBitmapButtons(16);
  ...
  //
  // The button owns two images that are swapped at run time (the padlock is
  // closed while remote input is blocked) and it is a check button (pressed =
  // remote input allowed). Both images live at the end of the toolbar strip.
  //
  m_imgRemoteInputLocked = IMG_REMOTEINPUT_LOCKED;
  m_imgRemoteInputEnabled = IMG_REMOTEINPUT_ENABLED;
  m_toolbar.addButton(m_imgRemoteInputLocked,
